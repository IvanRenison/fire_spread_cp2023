...
int main(int argc, char* argv[]) {
  try {
    ...
    std::srand(SEED);
    SimulationParams params = random_params();

    Matrix<uint> burned_amounts = burned_amounts_per_cell(
    ...
    for (uint i = 0; i < landscape.height; i++) {
      for (uint j = 0; j < landscape.width; j++) {
        if (j != 0) {
          std::cout << " ";
        }
